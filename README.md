...
3. Create a task instance and set the required [inputs](#inputs):
    ...
    chip_strips.inputs.images = ', '.join([join(imagery_input, '1040010014BCA700.tif'), join(imagery_input, '1040010014800C00.tif')])
...
|-----------------|------------------------------------------------------------------|
| geojson | Directory: Contains a geojson file with geometries to extract from images. **Properties must include class names if the chips are to be used for training**. If a feature_id property is not provided it will be generated by the task and included in ref.geojson. |
|  images | String: Location of all images to extract AOI data from. If using image strips this will be in the form of full paths to wach image separated by commas, (bucket/path/to/image_1.tif, bucket/path/to/image_2.tif). This is how the vrt will know which image to extract pixels from. If using a FLAME mosaic this should be the path to a project directory (bucket/path/to/project/) |  
|  mosaic | String ('boolean'): True if images input is a mosaic (task will expect images to be a FLAME project directory). If False task will expect one or more paths to individual strips. |  
|  aws_access_key | String: AWS access key that has read rights to images bucket (only necessary if images are in a private bucket). |  
|  aws_secret_key | String: AWS secret access key with read rights to images bucket (only necessary if images are in a private bucket). |  
|  aws_session_token | String: AWS session token for images bucket. Required for images in the gbd-customer-data bucket. It is recommended to use a 36-hour token (gbdxtools currently defaults to 10 hours). |  
